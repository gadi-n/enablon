...
def test_edit_task(browser, textbox_element, action_chains, elements):
    input_tasks = utils.delete_task_text(browser, textbox_element, action_chains, 'old task', elements)
    for char in 'new task':
        input_tasks[1].send_keys(char)
    input_tasks[1].send_keys(Keys.ENTER)
    task = browser.find_elements(By.XPATH, elements['task_element'])
    assert task[0].text == f'new task', f"Error! Task text is {task[0].text} and not 'new task'"


def test_complete_task_and_display_by_status(browser, textbox_element, elements):
    # preconditions - create tasks
    utils.create_tasks(browser, 5, 'task', textbox_element)
    checkboxes = (browser.find_elements(By.XPATH, elements['checkbox_element']))
    # complete tasks
    checkboxes[0].click()
    checkboxes[2].click()
    # show only completed
    completed_tasks = utils.get_task_elements_by_status(browser, "Completed", elements)
    ...
    # show only active
    active_tasks = utils.get_task_elements_by_status(browser, "Active", elements)
    ...
    # show all tasks
    all_tasks = utils.get_task_elements_by_status(browser, "All", elements)
...
def test_clear_completed_tasks(browser, textbox_element, elements):
...
def test_left_items(browser, textbox_element, elements):
    # preconditions - create tasks
    utils.create_tasks(browser, 3, 'task', textbox_element)
    left_items_element = browser.find_elements(By.CLASS_NAME, elements['left_items_text_element'])[0].text
    # validate text
    assert left_items_element == '3 items left!'
    browser.find_elements(By.XPATH, elements['checkbox_element'])[0].click()
    assert browser.find_elements(By.CLASS_NAME, elements['left_items_text_element'])[0].text == '2 items left!'
    browser.find_elements(By.XPATH, elements['checkbox_element'])[1].click()
    assert browser.find_elements(By.CLASS_NAME, elements['left_items_text_element'])[0].text == '1 item left!'
    browser.find_elements(By.XPATH, elements['checkbox_element'])[0].click()
    assert browser.find_elements(By.CLASS_NAME, elements['left_items_text_element'])[0].text == '2 items left!'
    checkboxes = (browser.find_elements(By.XPATH, elements['checkbox_element']))
    ...
    for checkbox in checkboxes:
        ...
        if flag:
            checkbox.click()
    # validate text
    assert browser.find_elements(By.CLASS_NAME, elements['left_items_text_element'])[0].text == '0 items left!'


def test_create_1000_tasks(browser, textbox_element, elements):
...
def test_mark_all_tasks_as_completed(browser, textbox_element, elements):
    # preconditions - create tasks
    utils.create_tasks(browser, 3, 'task', textbox_element)
    # mark as done
